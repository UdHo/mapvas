...
impl CoordinateParser {
    #[must_use] pub fn new() -> Self {
        Self {
            ...
            dms_regex: Regex::new(
                r"^\s*(\d+)°\s*(\d+)'\s*([NS])\s*[,\s]\s*(\d+)°\s*(\d+)'\s*([EW])\s*$"
            ).unwrap(),
        }
    }
    ...
    #[must_use] pub fn parse_coordinate(&self, input: &str) -> Option<SearchResult> {
        ...
        if let Some(coord) = self.parse_decimal(input) {
            log::debug!("Successfully parsed decimal coordinate: {:.4}, {:.4}", coord.lat, coord.lon);
...
impl SearchProvider for NominatimProvider {
    ...
    async fn search(&self, query: &str) -> Result<Vec<SearchResult>> {
        ...
        let response = self.client
            .get(&url)
            .header("User-Agent", "MapVas/0.2.4 (https://github.com/UdHo/mapvas)")
            .recv_json::<Value>()
            .await
...
impl SearchProvider for CustomProvider {
    ...
    async fn search(&self, query: &str) -> Result<Vec<SearchResult>> {
        let url = self.url_template.replace("{query}", &urlencoding::encode(query));
