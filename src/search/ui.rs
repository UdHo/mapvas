...
impl SearchUI {
  ...
  pub fn ui(&mut self, ui: &mut egui::Ui, map_sender: &RoutingSender) {
    ...
    ui.group(|ui| {
      ui.vertical(|ui| {
        ...
        let input_response = self.render_search_input(ui);

        // Handle input events and search logic
        self.handle_input_events(ui, &input_response);

        // Show status (searching, empty query hint)
        self.show_search_status(ui);

        // Handle keyboard navigation for results
        if self.show_results && !self.results.is_empty() {
          self.handle_keyboard_navigation(ui, map_sender);
        }
        ...
        // Show controls (clear button, result count)
  ...
  fn render_search_input(&mut self, ui: &mut egui::Ui) -> egui::Response {
  ...
  fn handle_input_events(&mut self, ui: &mut egui::Ui, response: &egui::Response) {
    ...
    // Request repaint for debounce timing
  ...
  fn show_search_status(&mut self, ui: &mut egui::Ui) {
  ...
  fn handle_keyboard_navigation(&mut self, ui: &mut egui::Ui, map_sender: &RoutingSender) {
    ui.input(|i| {
      if i.key_pressed(egui::Key::ArrowDown) {
        ...
        log::debug!(
          "Enter pressed in search results, selected_index: {:?}",
          self.selected_index
        );
        if let Some(index) = self.selected_index {
          if index < self.results.len() {
            ...
            log::warn!(
              "Selected index {} out of bounds (results len: {})",
              index,
              self.results.len()
            );
          }
  ...
  fn select_result(&mut self, index: usize, map_sender: &RoutingSender) {
    log::debug!(
      "select_result called with index: {}, results len: {}",
      index,
      self.results.len()
    );
