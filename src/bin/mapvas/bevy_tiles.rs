...
use mapvas::{
  ...
  map::{
    ...
    tile_renderer::{
      RasterTileRenderer, TileImage, TileRenderError, TileRenderer, VectorTileRenderer,
      background_color, get_fill_color, get_road_styling, style_config, style_version,
    },
...
impl BevyTileLayer {
  ...
  fn request_native_vector_tile(
    ...
    runtime.spawn(async move {
      ...
      let (render_rx, _) = mapvas::render_scheduler::RENDER_SCHEDULER.submit(priority, move || {
        native_vector_tile_meshes(&tile, &tile_data)
      });

      let render_result = tokio::time::timeout(std::time::Duration::from_secs(30), render_rx).await;
      let meshes = match render_result {
        Ok(Ok(Ok(meshes))) => meshes,
        _ => {
          let _ = sender.send(BevyTileResult::Failed { generation, tile });
          return;
        }
    ...
    match result {
      ...
      BevyTileResult::NativeVectorReady {
        ...
        if let Some(entry) = layer
          .loaded_native_vector_tiles
          .insert(tile, BevyNativeVectorTileEntry { instances })
        {
...
fn native_vector_tile_meshes(
  tile: &Tile,
  data: &[u8],
) -> Result<Vec<NativeVectorTileMeshSpec>, TileRenderError> {
  ...
  Ok(native_vector_batches_to_mesh_specs(batches, tile_nw))
}
