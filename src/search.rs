...
impl SearchManager {
  ...
  pub fn new() -> Self {
    Self {
      providers: vec![Box::new(providers::NominatimProvider::new(None))],
    ...
    for config in configs {
      match config {
        ...
        SearchProviderConfig::Custom {
          ...
          providers.push(Box::new(providers::CustomProvider::new(
            name,
            url_template,
            headers,
          )));
  ...
  pub async fn search(&self, query: &str) -> Result<Vec<SearchResult>> {
    ...
    log::debug!(
      "Search completed for '{}': {} total results",
      query,
      all_results.len()
    );
    Ok(all_results)
