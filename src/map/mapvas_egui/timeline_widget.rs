...
impl TimelineWidget {
  ...
  pub fn set_step_size(&mut self, step_size: f32) {
    self.step_size = step_size.clamp(MIN_STEP_SIZE, MAX_STEP_SIZE);
  }
  ...
  pub fn toggle_interval_lock(&mut self) {
    self.interval_lock = match self.interval_lock {
  ...
  pub fn step_forward(&mut self) {
    if let (Some(start), Some(end)) = (self.interval_start, self.interval_end) {
      let step_duration = if self.step_size < 1.0 {
        #[allow(clippy::cast_possible_truncation)]
        let millis = (self.step_size * 1000.0) as i64;
        Duration::milliseconds(millis)
      } else {
        #[allow(clippy::cast_possible_truncation)]
        let seconds = self.step_size as i64;
        #[allow(clippy::cast_possible_truncation)]
        let millis = ((self.step_size % 1.0) * 1000.0) as i64;
        Duration::seconds(seconds) + Duration::milliseconds(millis)
      };

      match self.interval_lock {
        IntervalLock::None => {
  ...
  pub fn step_to_end(&mut self) {
    if let (Some(time_end), Some(interval_start), Some(interval_end)) =
      (self.time_end, self.interval_start, self.interval_end)
    {
      ...
      match self.interval_lock {
        IntervalLock::None => {
          ...
          self.interval_end = Some(time_end);
          self.interval_start = Some(time_end - interval_duration);
        }
        IntervalLock::Start => {
  ...
  fn speed_to_position(&self) -> f32 {
    (self.playback_speed - MIN_SPEED) / (MAX_SPEED - MIN_SPEED)
  }

  /// Calculate normalized position for step size slider (0.0 to 1.0)  
  fn step_size_to_position(&self) -> f32 {
    (self.step_size.ln() - MIN_STEP_SIZE.ln()) / (MAX_STEP_SIZE.ln() - MIN_STEP_SIZE.ln())
  }
  ...
  fn handle_handles_interaction(
      ...
      match self.interval_lock {
        IntervalLock::Start => {
          if ui.button("🔓 Unlock Start").clicked() {
            self.interval_lock = IntervalLock::None;
            ui.close();
          }
        }
        _ => {
          if ui.button("🔒 Lock Start").clicked() {
            self.interval_lock = IntervalLock::Start;
            ui.close();
          }
        }
      }
    ...
    if self.interval_lock != IntervalLock::Start {
      ...
      match self.interval_lock {
        ...
        _ => {
          if ui.button("🔒 Lock End").clicked() {
            self.interval_lock = IntervalLock::End;
            ui.close();
          }
        }
      }
    ...
    // Handle dragging only if not locked
    if self.interval_lock != IntervalLock::End {
  ...
  fn handle_step_slider_interaction(
    ...
    if step_response.dragged() {
      if let Some(pointer_pos) = ui.ctx().pointer_interact_pos() {
        ...
        self.step_size = log_value.exp().clamp(MIN_STEP_SIZE, MAX_STEP_SIZE);
      }
    }
  }
  ...
  fn draw_timeline_track(&mut self, ui: &mut Ui, track_rect: Rect) {
    ...
    if let (Some(interval_start), Some(interval_end)) = (self.interval_start, self.interval_end) {
      if let (Some(start_pos), Some(end_pos)) = (
        ...
        let interval_color = if self.dragging_interval {
          interval_drag_color()
  ...
  fn draw_step_control(
    ...
    // Step label
    let step_text = if self.step_size < 1.0 {
      format!("Step: {:.1}s", self.step_size)
    } else if self.step_size < 60.0 {
      if self.step_size.fract() == 0.0 {
        #[allow(clippy::cast_possible_truncation)]
        let seconds = self.step_size as i32;
  ...
  fn draw_lock_button(
    ...
    let lock_text = match self.interval_lock {
      IntervalLock::None => "🔓",
  ...
  fn draw_interval_handles(
    ...
    // Start handle
    let start_color = if self.interval_lock == IntervalLock::Start {
      Color32::from_rgb(255, 165, 0) // Orange for locked
    } else if self.dragging_start {
      handle_hover_color()
    } else {
      handle_normal_color()
    };
    ...
    // End handle
    let end_color = if self.interval_lock == IntervalLock::End {
      Color32::from_rgb(255, 165, 0) // Orange for locked
    } else if self.dragging_end {
      handle_hover_color()
    } else {
      handle_normal_color()
    };
...
mod tests {
  ...
  fn test_step_forward_with_start_lock() {
    ...
    // Lock the start
    widget.interval_lock = IntervalLock::Start;
  ...
  fn test_step_size_logarithmic_scaling() {
    ...
    test_widget.step_size = MIN_STEP_SIZE;
    ...
    test_widget.step_size = MAX_STEP_SIZE;
  ...
  fn test_step_size_conversion_roundtrip() {
    ...
    for original_step_size in test_cases {
      let mut widget = TimelineWidget::new();
      widget.step_size = original_step_size;
  ...
  fn test_step_duration_precision() {
    ...
    for (step_size, expected_duration) in test_cases {
      ...
      let calculated_duration = if widget.step_size < 1.0 {
        #[allow(clippy::cast_possible_truncation)]
        {
          Duration::milliseconds((widget.step_size * 1000.0) as i64)
        }
      } else {
        #[allow(clippy::cast_possible_truncation)]
        {
          Duration::seconds(widget.step_size as i64)
            + Duration::milliseconds(((widget.step_size % 1.0) * 1000.0) as i64)
        }
      };
