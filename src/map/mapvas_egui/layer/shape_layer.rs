...
impl ShapeLayer {
  ...
  fn show_shape_ui(
    ...
    let is_scroll_target = self.just_double_clicked.as_ref().is_some_and(|(l, idx, _)| {
      l == layer_id && *idx == shape_idx
    });
  ...
  fn show_nested_geometry_content(
    ...
    if let Geometry::GeometryCollection(nested_geometries, nested_metadata) = geometry {
      ...
      if is_highlighted {
        ...
        if self.just_double_clicked.as_ref().is_some_and(|(l, idx, path)| {
          l == layer_id && *idx == shape_idx && path == nested_path
        }) {
          horizontal_response
            .response
