...
use crate::{
  map::{
    coordinates::{BoundingBox, Coordinate as _},
...
impl CurlCfg {
    ...
    match parameters {
      ParameterUpdate::Update(key, coord) => {
        if let Some(coord) = coord {
          if let Some(OoMCoordinates::Coordinate(old)) = self.coordinates.get_mut(&key) {
            *old = coord;
            return true;
          }
        }
  ...
  fn bounding_box(&self) -> BoundingBox {
    BoundingBox::from_iterator(self.coordinates.values().flat_map(|coord| match coord {
...
impl ExCommand {
  ...
  fn coordinates(&self) -> &HashMap<String, OoMCoordinates> {
    match self {
